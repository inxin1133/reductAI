...
> 이 문서는 reductai 시스템에 Claude Sonnet 4.5를 등록할 때 참조하는 설정 스펙입니다.
...
| 필드 | 비고 |
|------|------|
| name | 예: `claude-sonnet-4-5-chat-block-json` |
| purpose | `chat` |
| is_active | `true` |
| body | 아래 JSON |
...
| model_id | `claude-sonnet-4-5-20250929` | API 모델 ID. 별칭 `claude-sonnet-4-5` 사용 가능 |
| display_name | `Claude Sonnet 4.5` | 표시용 |
| model_type | `text` | |
| context_window | `200000` | 기본 200K. 1M은 베타(헤더 `anthropic-beta: cache-v1`) |
| max_input_tokens | `200000` | models_prompt_text(chat).md 권장 |
| max_output_tokens | `65536` | 최대 64K 출력 ([모델 문서](https://platform.claude.com/docs/models-overview)) |
...
> **prompt_caching**: `cache_control: { type: "ephemeral" }` 또는 `{ type: "ephemeral", ttl: "1h" }` 지원. reductai `anthropicSimulateChat`의 `cacheControl` 옵션으로 전달됨.  
> **extended_thinking**: Claude Sonnet 4.5는 extended thinking 지원. `thinking` 파라미터로 제어 가능 (공식 문서 참조).  
> **가격**: 입력 $3/MTok, 출력 $15/MTok ([Pricing](https://www.anthropic.com/pricing)).
...
| target_model_id | (ai_models.id) | Claude Sonnet 4.5 모델 ID |
