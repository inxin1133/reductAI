...
> 이 문서는 reductai 시스템에 Gemini 2.5 Flash Preview TTS(텍스트→음성 모델)를 등록할 때 참조하는 설정 스펙입니다.
...
Gemini 2.5 Flash Preview TTS (`gemini-2.5-flash-preview-tts`)는 Google의 **저지연 텍스트→음성** 모델로, **Gemini API**를 통해 제공됩니다.  
비용 효율적이며, 팟캐스트·오디오북·고객 지원 등 일상적인 TTS 용도에 최적화되어 있습니다.
...
| 필드 | 값 | 비고 |
|------|-----|------|
| profile_key | `google_gemini_api_key_v1` | 예시 (Pro TTS와 공유 가능) |
| auth_type | `api_key` | |
...
> `{{userPrompt}}`: 사용자 입력(변환할 텍스트). 스타일 지시 포함 가능.
...
| 필드 | 값 | 비고 |
|------|-----|------|
| name | `gemini-2.5-flash-preview-tts` | reductai 모델 이름 |
| model_id | `gemini-2.5-flash-preview-tts` | API 모델 ID. [모델 목록](https://ai.google.dev/gemini-api/docs/models) 참고 |
| display_name | `Gemini 2.5 Flash TTS` | 표시용 |
...
```json
{
  ...
  "options": {
    "voice": {
      "type": "string",
      "label": "voice",
      "description": "음성 이름. Kore, Aoede 등. 공식 문서 참고"
    },
    "format": {
      "type": "enum",
      "label": "format",
    ...
    "style_prompt": {
      "type": "string",
      "label": "style_prompt",
      "description": "자연어 스타일 지시 (예: Say cheerfully)"
    }
  },
  "defaults": {
    "voice": "Kore",
  ...
  "validation_hints": [
    "Flash TTS는 저지연·비용 효율에 최적화. Pro TTS보다 응답 속도 빠름.",
    "30+ 스피커, 80+ 로케일 지원. 멀티 스피커(2명) 지원."
  ]
}
```
...
> - 음성: Kore, Aoede 등 30+ 스피커  
> - GA: 2025년 9월 30일 (일반 제공)  
...
> `{{userPrompt}}`: 사용자 입력.  
> `{{params_voice}}`: capabilities.options.voice (기본: Kore).
...
| target_model_id | (ai_models.id) | Gemini 2.5 Flash TTS 모델 ID |
| fallback_model_id | (선택) | 예: gemini-2.5-pro-preview-tts |
| priority | `0` | |
...
6. **model_api_profiles**: purpose=`audio`, profile_key=`google_gemini_flash_tts_v1`, credential 연동
...
| 기본 음성 | Callirrhoe | Kore |
| 지연 시간 | 상대적으로 높음 | 낮음 |
| 과금 | Pro 단가 | Flash 단가 (저렴) |

> [gemini-2.5-pro-preview-tts.md](gemini-2.5-pro-preview-tts.md) 참고.
...
- **시스템 통합**: 현재 audio는 OpenAI만 내장 지원합니다. Gemini TTS 사용 시 **model_api_profiles**(purpose=audio)가 반드시 필요합니다.
