...
> 이 문서는 reductai 시스템에 Gemini 3 Pro를 등록할 때 참조하는 설정 스펙입니다.
...
| 필드 | 값 | 비고 |
|------|-----|------|
| name | `gemini-3-pro` | 모델 이름 |
| model_id | `gemini-3-pro` | API 모델 ID. [모델 목록](https://ai.google.dev/gemini-api/docs/models)에서 확인 (예: gemini-2.5-pro, gemini-3-pro-preview) |
| display_name | `Gemini 3 Pro` | 표시용 |
| model_type | `text` | |
| context_window | `1048576` | models_prompt_text(chat).md 권장 |
| max_input_tokens | `850000` | models_prompt_text(chat).md 권장 |
| max_output_tokens | `65536` | models_prompt_text(chat).md 권장 |
...
> models_prompt: markdown 블록은 `markdown` 필드 사용 (content 아님). code 블록은 `code` 필드 사용.
...
| 필드 | 비고 |
|------|------|
| name | 예: `gemini-3-pro-chat-block-json` |
| purpose | `chat` |
| is_active | `true` |
| body | 아래 JSON |
...
```
> systemInstruction의 `parts`는 text만 지원. generationConfig 파라미터: [GenerationConfig](https://cloud.google.com/vertex-ai/docs/reference/rest/v1/GenerationConfig) 참고.
...
| target_model_id | (ai_models.id) | Gemini 3 Pro 모델 ID |
...
1. **ai_providers**: Google provider가 없으면 생성
2. **provider_api_credentials**: API Key 등록 (x-goog-api-key)
3. **response_schemas**: block_json 스키마 생성 → ID 확보
4. **prompt_templates**: systemInstruction 형식 프롬프트 생성 → ID 확보
