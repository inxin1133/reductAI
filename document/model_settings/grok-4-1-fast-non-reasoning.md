...
> 이 문서는 reductai 시스템에 Grok 4.1 Fast **Non-Reasoning**(비추론형) 모델을 등록할 때 참조하는 설정 스펙입니다.
> reasoning 모델과 달리 **확장 추론(reasoning tokens)을 사용하지 않아** 응답 속도가 빠르고, 단순·직접적인 태스크에 적합합니다.
...
| 필드 | 비고 |
|------|------|
| name | 예: `grok-4-1-fast-non-reasoning-chat-block-json` |
| purpose | `chat` |
| is_active | `true` |
| body | 아래 JSON |
...
| 필드 | 값 | 비고 |
|------|-----|------|
| name | `grok-4-1-fast-non-reasoning` | 모델 이름 |
| model_id | `grok-4-1-fast-non-reasoning` | API 모델 ID. [Models](https://docs.x.ai/developers/models) 참조 |
| display_name | `Grok 4.1 Fast (Non-Reasoning)` | 표시용. 비추론형—확장 thinking 미사용 |
...
> **가격**: 입력 $0.20/MTok, 캐시 입력 $0.05/MTok, 출력 $0.50/MTok ([Models & Pricing](https://docs.x.ai/developers/models)). reasoning 모델과 동일.  
> **Rate limits**: 4M TPM, 480 RPM.  
> **reasoning: false**: 비추론형. 확장 추론(reasoning tokens)을 사용하지 않아 응답이 빠르고, 복잡한 사고 연쇄가 필요 없는 태스크에 적합합니다.  
> **Capabilities**: functions, structured (reasoning 미포함). [Models](https://docs.x.ai/developers/models) 참조.  
> **prompt_caching**: 자동 활성화.
...
> **Non-Reasoning**: 추론 토큰 미사용으로 응답이 빠르며, `timeout_ms` 30초 정도로 충분한 경우가 많습니다.
...
| target_model_id | (ai_models.id) | Grok 4.1 Fast (Non-Reasoning) 모델 ID |
| fallback_model_id | (선택) | 예: grok-4-1-fast-reasoning (복잡한 추론 필요 시 업그레이드) |
| priority | `0` | |
