...
-- SSO 이메일 인증 대기(네이버/카카오 등 이메일 미제공/미인증 대응)
...
CREATE INDEX idx_sso_pending_provider_user ON sso_pending(provider, provider_user_id);
CREATE INDEX idx_sso_pending_token ON sso_pending(sso_token);
CREATE INDEX idx_sso_pending_status ON sso_pending(status);

COMMENT ON TABLE sso_pending IS 'SSO 이메일 입력/인증 완료 전 임시 상태 저장';
COMMENT ON COLUMN sso_pending.provider IS 'SSO 제공자 (google/kakao/naver)';
COMMENT ON COLUMN sso_pending.provider_user_id IS '프로바이더별 유저 ID';
COMMENT ON COLUMN sso_pending.email IS '사용자가 입력 중인 이메일(미인증 가능)';
COMMENT ON COLUMN sso_pending.status IS '상태(pending/verified/expired)';
COMMENT ON COLUMN sso_pending.sso_token IS '이메일 인증 흐름을 위한 임시 토큰';
COMMENT ON COLUMN sso_pending.expires_at IS '만료 시각';
COMMENT ON COLUMN sso_pending.metadata IS '프로필 정보 등 추가 데이터(JSON)';
...
COMMENT ON COLUMN user_tenant_roles.left_by IS '테넌트 탈퇴 처리자 사용자 ID (NULL이면 본인 탈퇴 또는 미기록)';
