...
function extractOptionalUserIdFromAuthHeader(req) {
    try {
...
async function chatCompletion(req, res) {
    try {
        ...
        const provider = await (0, db_1.query)(`SELECT id, slug, provider_family, api_base_url FROM ai_providers WHERE slug = $1`, [provider_slug]);
        ...
        const providerKey = (0, providerClientKey_1.deriveProviderClientKey)(providerRow.provider_family, providerRow.slug);
        ...
        if (providerKey === "openai") {
            ...
            `, [modelRow.id]);
                    ...
                    if (row?.name && schema && typeof schema === "object" && !Array.isArray(schema)) {
                        responseSchema = { name: String(row.name), strict: Boolean(row.strict), schema: schema };
                    }
                }
            }
            catch (e) {
        ...
        try {
            const { provider_slug = "openai", model, input, max_tokens = 512, output_format } = req.body || {};
            const provider = await (0, db_1.query)(`SELECT id FROM ai_providers WHERE slug = $1`, [provider_slug]);
