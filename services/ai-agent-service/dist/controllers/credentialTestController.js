...
exports.testSora2Access = testSora2Access;
const db_1 = require("../config/db");
const cryptoService_1 = require("../services/cryptoService");
function inferBaseUrl(endpointUrl) {
...
async function testSora2Access(req, res) {
    try {
        const { id } = req.params;
        const modelId = String(req.query.model_id || "sora-2").trim() || "sora-2";
        if (!id)
            return res.status(400).json({ message: "credential id is required" });
        const r = await (0, db_1.query)(`SELECT id, provider_id, credential_name, api_key_encrypted, endpoint_url, organization_id, is_active, metadata
       ...
       LIMIT 1`, [id]);
        ...
        if (cred.is_active === false)
            return res.status(400).json({ message: "Credential is not active" });
        const apiKey = (0, cryptoService_1.decryptApiKey)(String(cred.api_key_encrypted || ""));
