...
function maskedFromLast4(last4) {
    if (!last4)
        return null;
    return `••••••••••${last4}`;
}
...
async function createCredential(req, res) {
    ...
    try {
        const { provider_id, credential_name, api_key, endpoint_url = null, organization_id = null, is_active = true, is_default = false, rate_limit_per_minute = null, rate_limit_per_day = null, metadata = {}, expires_at = null, } = req.body;
        ...
        const tenant_id = await (0, systemTenantService_1.ensureSystemTenantId)();
        // 보안: DB에는 암호화/해시만 저장
        const encrypted = (0, cryptoService_1.encryptApiKey)(api_key);
        const hash = (0, cryptoService_1.sha256)(api_key);
        ...
        if (is_default) {
            await client.query(`UPDATE provider_api_credentials
         SET is_default = FALSE, updated_at = CURRENT_TIMESTAMP
         WHERE tenant_id = $1 AND provider_id = $2 AND is_default = TRUE`, [tenant_id, provider_id]);
        }
        const insert = await client.query(`INSERT INTO provider_api_credentials
    ...
    try {
       ...
       WHERE id = $1`, [id]);
        ...
        if (api_key && api_key.trim()) {
            encrypted = (0, cryptoService_1.encryptApiKey)(api_key);
            hash = (0, cryptoService_1.sha256)(api_key);
        ...
        if (is_default === true) {
            await client.query(`UPDATE provider_api_credentials
         SET is_default = FALSE, updated_at = CURRENT_TIMESTAMP
         WHERE tenant_id = $1 AND provider_id = $2 AND is_default = TRUE AND id <> $3`, [curTenantId, curProviderId, id]);
        }
        const result = await client.query(`UPDATE provider_api_credentials SET
        tenant_id = $2,
      ...
      RETURNING
        ...
        const metaLast4 = row.metadata?.last4 || null;
...
async function deleteCredential(req, res) {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`DELETE FROM provider_api_credentials WHERE id = $1 RETURNING id`, [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "Credential not found" });
        res.json({ ok: true });
    }
