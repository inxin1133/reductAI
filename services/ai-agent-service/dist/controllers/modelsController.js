...
exports.reorderModels = reorderModels;
...
async function getModels(req, res) {
    try {
        ...
        if (q) {
            ...
            where.push(`(m.display_name ILIKE $${params.length} OR m.model_id ILIKE $${params.length} OR p.product_name ILIKE $${params.length})`);
        }
      ...
      ${whereSql}
      ORDER BY m.model_type ASC, m.sort_order ASC, p.product_name ASC, m.display_name ASC`, params);
...
async function createModel(req, res) {
    try {
        const { provider_id, name, model_id, display_name, description = null, model_type, prompt_template_id = null, response_schema_id = null, capabilities = {}, context_window = null, max_output_tokens = null, input_token_cost_per_1k = 0, output_token_cost_per_1k = 0, currency = "USD", is_available = true, is_default = false, status = "active", released_at = null, deprecated_at = null, sort_order = null, metadata = {}, } = req.body;
       ...
       RETURNING *`, [
            ...
            typeof sort_order === "number"
                ? sort_order
                : (await (0, db_1.query)(`SELECT COALESCE(MAX(sort_order), 0) AS max FROM ai_models WHERE model_type = $1`, [model_type])).rows?.[0]?.max + 10,
...
async function updateModel(req, res) {
    try {
        ...
        const body = req.body || {};
        // 부분 업데이트
        const result = await (0, db_1.query)(`UPDATE ai_models SET
...
async function reorderModels(req, res) {
    const client = await db_1.default.connect();
    try {
        const { model_type, ordered_ids } = (req.body || {});
       ...
       FROM ai_models
       WHERE model_type = $1
         AND id = ANY($2::uuid[])`, [model_type, ids]);
...
async function deleteModel(req, res) {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`DELETE FROM ai_models WHERE id = $1 RETURNING id`, [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "Model not found" });
...
async function simulateModel(req, res) {
    try {
        const { model_id, input, max_tokens = 128 } = req.body;
        if (!model_id || !input)
            return res.status(400).json({ message: "model_id and input are required" });
        const m = await (0, db_1.query)(`SELECT m.id, m.model_id AS model_api_id, m.provider_id, p.provider_family, p.name AS provider_name, p.slug AS provider_slug, p.api_base_url
       FROM ai_models m
       JOIN ai_providers p ON p.id = m.provider_id
       WHERE m.id = $1`, [model_id]);
        if (m.rows.length === 0)
            return res.status(404).json({ message: "Model not found" });
        const row = m.rows[0];
        ...
        if (providerKey === "anthropic") {
            const out = await (0, providerClients_1.anthropicSimulateChat)({
                apiKey: auth.apiKey,
                model: modelApiId,
                input,
                maxTokens: Number(max_tokens) || 128,
        ...
        if (providerKey === "google") {
            const out = await (0, providerClients_1.googleSimulateChat)({
                apiBaseUrl: auth.endpointUrl || apiBaseUrl,
                apiKey: auth.apiKey,
                model: modelApiId,
                input,
