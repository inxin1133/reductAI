...
function normalizeScope(tenantId, scope_type, scope_id) {
    const stRaw = typeof scope_type === "string" ? scope_type.trim().toUpperCase() : "";
    const st = stRaw === "GLOBAL" || stRaw === "ROLE" || stRaw === "TENANT" ? stRaw : "TENANT";
    if (st === "GLOBAL")
        return { scope_type: "GLOBAL", scope_id: null };
    if (st === "TENANT")
        return { scope_type: "TENANT", scope_id: tenantId };
    // ROLE
...
async function listRoutingRules(req, res) {
    try {
        ...
        const countRes = await (0, db_1.query)(`SELECT COUNT(*)::int AS total FROM model_routing_rules r ${whereSql}`, params);
...
async function getRoutingRule(req, res) {
    try {
        const tenantId = await (0, systemTenantService_1.ensureSystemTenantId)();
...
async function createRoutingRule(req, res) {
    try {
        const tenantId = await (0, systemTenantService_1.ensureSystemTenantId)();
        const { scope_type, scope_id, rule_name, priority = 0, conditions, target_model_id, fallback_model_id = null, is_active = true, metadata = {}, } = req.body || {};
        const name = String(rule_name || "").trim();
        if (!name)
            return res.status(400).json({ message: "rule_name is required" });
        if (!target_model_id)
            return res.status(400).json({ message: "target_model_id is required" });
      ...
      `, [
        ...
        if (e?.code === "23505")
            return res.status(409).json({ message: "Duplicate rule_name in scope (already exists)" });
...
async function updateRoutingRule(req, res) {
    try {
        ...
        const id = String(req.params.id || "");
        const body = req.body || {};
        const existing = await (0, db_1.query)(`SELECT id FROM model_routing_rules WHERE tenant_id = $1 AND id = $2`, [tenantId, id]);
...
async function deleteRoutingRule(req, res) {
    try {
        const tenantId = await (0, systemTenantService_1.ensureSystemTenantId)();
        const id = String(req.params.id || "");
        const r = await (0, db_1.query)(`DELETE FROM model_routing_rules WHERE tenant_id = $1 AND id = $2 RETURNING id`, [tenantId, id]);
        if (r.rows.length === 0)
            return res.status(404).json({ message: "Not found" });
