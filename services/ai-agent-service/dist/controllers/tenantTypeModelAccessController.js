...
exports.getTypeModelAccess = getTypeModelAccess;
exports.getTypeModelAccessItem = getTypeModelAccessItem;
exports.createTypeModelAccess = createTypeModelAccess;
exports.updateTypeModelAccess = updateTypeModelAccess;
exports.deleteTypeModelAccess = deleteTypeModelAccess;
...
async function getTypeModelAccess(req, res) {
    try {
      ...
      ORDER BY a.priority DESC, p.product_name ASC, m.display_name ASC`, [tenant_type]);
...
async function getTypeModelAccessItem(req, res) {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`SELECT
        a.*,
...
// 생성
async function createTypeModelAccess(req, res) {
    const client = await db_1.default.connect();
    try {
        const { tenant_type, model_id, credential_id = null, status = "active", access_level = "standard", priority = 0, is_preferred = false, rate_limit_per_minute = null, rate_limit_per_day = null, max_tokens_per_request = null, allowed_features = [], metadata = {}, } = req.body;
        ...
        if (credential_id) {
            ...
            const check = await client.query(`SELECT id FROM provider_api_credentials WHERE id = $1 AND tenant_id = $2`, [credential_id, systemTenantId]);
        ...
        if (is_preferred) {
            await client.query(`UPDATE tenant_type_model_access
         SET is_preferred = FALSE, updated_at = CURRENT_TIMESTAMP
         WHERE tenant_type = $1 AND is_preferred = TRUE`, [tenant_type]);
        }
        const result = await client.query(`INSERT INTO tenant_type_model_access
...
// 수정
async function updateTypeModelAccess(req, res) {
    const client = await db_1.default.connect();
    try {
        const { id } = req.params;
        const { credential_id, status, access_level, priority, is_preferred, rate_limit_per_minute, rate_limit_per_day, max_tokens_per_request, allowed_features, metadata, } = req.body;
        const cur = await client.query(`SELECT id, tenant_type FROM tenant_type_model_access WHERE id = $1`, [id]);
        if (cur.rows.length === 0)
            return res.status(404).json({ message: "Item not found" });
        const tenantType = cur.rows[0].tenant_type;
        if (credential_id) {
            const systemTenantId = await (0, systemTenantService_1.ensureSystemTenantId)();
            const check = await client.query(`SELECT id FROM provider_api_credentials WHERE id = $1 AND tenant_id = $2`, [credential_id, systemTenantId]);
            if (check.rows.length === 0) {
        ...
        if (is_preferred === true) {
            await client.query(`UPDATE tenant_type_model_access
         SET is_preferred = FALSE, updated_at = CURRENT_TIMESTAMP
         WHERE tenant_type = $1 AND is_preferred = TRUE AND id <> $2`, [tenantType, id]);
        }
        const result = await client.query(`UPDATE tenant_type_model_access SET
...
async function deleteTypeModelAccess(req, res) {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`DELETE FROM tenant_type_model_access WHERE id = $1 RETURNING id`, [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "Item not found" });
        res.json({ ok: true });
    }
