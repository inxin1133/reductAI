...
function extractUsageFromProviderRaw(raw: any): { input_tokens: number; output_tokens: number; total_tokens: number } {
...
export async function chatCompletion(req: Request, res: Response) {
  try {
    ...
    if (provider_slug === "openai") {
      const effectiveInput = output_format === "block_json" ? blockJsonInstruction(input) : input
