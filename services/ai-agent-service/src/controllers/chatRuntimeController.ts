...
function extractTokenBreakdown(u: any): UsageTokenBreakdown | null {
  ...
  if (inText === 0 && inImage === 0 && outText === 0 && outImage === 0) return null
...
function sanitizeUsageLogRaw(raw: unknown, maxChars = MAX_USAGE_LOG_STRING): unknown {
  if (!raw || typeof raw !== "object") return raw
  try {
    const seen = new WeakSet<object>()
    const replacer = (_key: string, value: unknown) => {
      if (typeof value === "string") {
        if (value.startsWith("data:")) return `data:[omitted,len=${value.length}]`
        if (value.length > maxChars) return `${value.slice(0, maxChars)}…(len=${value.length})`
        return value
      }
  ...
  if (typeof input === "string") {
    ...
    if (exact) {
      ...
      if (k === "params_seconds") {
        ...
        if (vars.__force_seconds_string === true) return s
      }
...
async function executeHttpJsonProfile(args: {
  ...
  if (pickString(workflow, "type") === "async_job") {
    ...
    if (urlStr && urlStr.startsWith("gs://")) {
      try {
        const signed = await gcsUriToSignedUrl(urlStr)
        if (signed) urlStr = signed
      } catch (e) {
        console.warn("[Veo] gcsUri to signed URL failed, passing through:", (e as Error)?.message)
      }
    }

    const videoPayload =
      dataUrl ? { mime, data_url: dataUrl } : urlStr ? { url: urlStr } : {}
...
export async function chatRun(req: Request, res: Response) {
  ...
  let imageUsage: { count: number; size?: string; quality?: string; resolution?: string } | null = null
  let videoUsage: { seconds: number; size?: string; resolution?: string } | null = null
  ...
  try {
    ...
    if (mt === "video" && out == null) {
      ...
      try {
        const diagProfiles = await query(
          `SELECT id, tenant_id, provider_id, model_id, profile_key, purpose, is_active
           FROM model_api_profiles
        ...
        diagnostic = {
          ...
          video_profiles_for_provider: (diagProfiles.rows || []).map((row: Record<string, unknown>) => ({
            id: row.id,
    ...
    if (mt === "video" && out && !videoUsage) {
      const contentOpts = isRecord((out.content as Record<string, unknown>)?.options)
        ? ((out.content as Record<string, unknown>).options as Record<string, unknown>)
        : null
      const secondsFromOpts =
        mergedOptions?.seconds ??
        (mergedOptions as Record<string, unknown> | null)?.duration ??
        (mergedOptions as Record<string, unknown> | null)?.duration_seconds ??
        contentOpts?.seconds ??
        (contentOpts as Record<string, unknown> | null)?.duration ??
        (out.content as Record<string, unknown>)?.seconds ??
        (out.content as Record<string, unknown>)?.duration
      const seconds =
        typeof secondsFromOpts === "number"
      ...
      const size =
        typeof mergedOptions?.size === "string"
          ...
          : typeof (out.content as Record<string, unknown>)?.size === "string"
            ...
            : undefined
      const resolution =
        typeof mergedOptions?.resolution === "string"
          ? String(mergedOptions.resolution).trim()
          : typeof (out.content as Record<string, unknown>)?.resolution === "string"
            ? String((out.content as Record<string, unknown>).resolution).trim()
            : undefined
      const rawDuration =
        getByPath(out?.raw, "download.response.videos[0].durationSeconds") ??
      ...
      const hasVideo = isRecord((out.content as Record<string, unknown>)?.video)
    ...
    try {
      if (usedProviderId && usedModelDbId && usedModelApiId) {
        ...
        let costs: { inputCost: number; cachedInputCost: number; outputCost: number; totalCost: number; currency: string }
        if (hasTokenBreakdown && usage.token_breakdown) {
          const imagePricing = await lookupImageTokenPricing(usedProviderSlug, usedModelApiId, usedModelDbId)
          costs = calculateImageTokenCost(imagePricing, usage.token_breakdown)
        } else {
          ...
          costs = calculateCost(pricing, inputTokens, cachedInputTokens, outputTokens)
        }
        ...
        const imageCost =
          !hasTokenBreakdown && modality === "image_create" && imageUsage && imageUsage.count > 0
        ...
        const videoCost =
          modality === "video" && videoUsage && videoUsage.seconds > 0
            ? (await lookupVideoPricing(
                usedProviderSlug,
                usedModelApiId,
                videoUsage.resolution ?? videoUsage.size ?? null,
                usedModelDbId,
              )) * videoUsage.seconds
            : 0
        const audioCost = 0
        ...
        if (usageLogId) {
          if (inputTokens > 0 || cachedInputTokens > 0 || outputTokens > 0) {
            if (hasTokenBreakdown && usage.token_breakdown) {
              await query(`DELETE FROM llm_token_usages WHERE usage_log_id = $1`, [usageLogId])
              const b = usage.token_breakdown
              if (b.input_text_tokens > 0 || b.output_text_tokens > 0) {
                await query(
                  `INSERT INTO llm_token_usages (usage_log_id, input_tokens, cached_input_tokens, output_tokens, unit, token_category)
                   VALUES ($1, $2, 0, $3, 'tokens', 'text')`,
                  [usageLogId, b.input_text_tokens, b.output_text_tokens]
                )
              }
              if (b.input_image_tokens > 0 || b.output_image_tokens > 0) {
                await query(
                  `INSERT INTO llm_token_usages (usage_log_id, input_tokens, cached_input_tokens, output_tokens, unit, token_category)
                   VALUES ($1, $2, 0, $3, 'tokens', 'image')`,
                  [usageLogId, b.input_image_tokens, b.output_image_tokens]
                )
              }
            } else {
              await query(
