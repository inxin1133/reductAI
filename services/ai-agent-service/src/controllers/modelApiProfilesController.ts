...
function normalizePurpose(v: unknown): Purpose | null {
  const s = typeof v === "string" ? v.trim() : ""
  const allowed = new Set<Purpose>(["chat", "image", "video", "audio", "music", "multimodal", "embedding", "code"])
  return allowed.has(s as Purpose) ? (s as Purpose) : null
}

export async function listModelApiProfiles(req: Request, res: Response) {
  try {
    ...
    const countRes = await query(`SELECT COUNT(*)::int AS total FROM model_api_profiles ${whereSql}`, params)
...
export async function getModelApiProfile(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const id = String(req.params.id || "")
    const r = await query(`SELECT * FROM model_api_profiles WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, id])
...
export async function createModelApiProfile(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const body = req.body || {}
    ...
    if (Object.keys(transport).length === 0) return res.status(400).json({ message: "transport (JSON object) is required" })
    if (Object.keys(responseMapping).length === 0) return res.status(400).json({ message: "response_mapping (JSON object) is required" })
    ...
    if (e?.code === "23505") return res.status(409).json({ message: "Duplicate profile (tenant/provider/profile_key already exists)" })
    return res.status(500).json({ message: "Failed to create model api profile", details: String(e?.message || e) })
  }
}

export async function updateModelApiProfile(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const id = String(req.params.id || "")
    const body = req.body || {}

    const existing = await query(`SELECT id FROM model_api_profiles WHERE tenant_id = $1 AND id = $2`, [tenantId, id])
    ...
    if (body.purpose !== undefined) {
      ...
      if (!p) return res.status(400).json({ message: "purpose is invalid" })
      params.push(p)
    ...
    if (body.transport !== undefined) {
      ...
      if (Object.keys(t).length === 0) return res.status(400).json({ message: "transport must be a JSON object" })
    ...
    if (body.response_mapping !== undefined) {
      ...
      if (Object.keys(rm).length === 0) return res.status(400).json({ message: "response_mapping must be a JSON object" })
    ...
    if (fields.length === 1) {
      const row = await query(`SELECT * FROM model_api_profiles WHERE tenant_id = $1 AND id = $2`, [tenantId, id])
...
export async function deleteModelApiProfile(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const id = String(req.params.id || "")
    const r = await query(`DELETE FROM model_api_profiles WHERE tenant_id = $1 AND id = $2 RETURNING id`, [tenantId, id])
    if (r.rows.length === 0) return res.status(404).json({ message: "Not found" })
