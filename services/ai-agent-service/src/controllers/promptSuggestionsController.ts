...
function normalizeScopeType(v: unknown): "GLOBAL" | "TENANT" | "ROLE" {
  const s = String(v || "").trim().toUpperCase()
  if (s === "GLOBAL" || s === "ROLE" || s === "TENANT") return s
  return "TENANT"
}

function normalizeModelType(v: unknown): string | null {
...
export async function listPromptSuggestions(req: Request, res: Response) {
  try {
    ...
    const countRes = await query(`SELECT COUNT(*)::int AS total FROM prompt_suggestions ${whereSql}`, params)
...
export async function getPromptSuggestion(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const id = String(req.params.id || "")
    const r = await query(`SELECT * FROM prompt_suggestions WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, id])
...
export async function createPromptSuggestion(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const body = req.body || {}
    ...
    if (scopeType === "GLOBAL" && scopeId) return res.status(400).json({ message: "scope_id must be null when scope_type=GLOBAL" })
...
export async function updatePromptSuggestion(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const id = String(req.params.id || "")
    const body = req.body || {}

    const existing = await query(`SELECT id FROM prompt_suggestions WHERE tenant_id = $1 AND id = $2`, [tenantId, id])
    ...
    if (body.scope_type !== undefined || body.scope_id !== undefined) {
      const scopeType = normalizeScopeType(body.scope_type)
      const scopeId =
        body.scope_id === undefined || body.scope_id === null || String(body.scope_id).trim() === "" ? null : String(body.scope_id).trim()
    ...
    if (fields.length === 1) {
      const row = await query(`SELECT * FROM prompt_suggestions WHERE tenant_id = $1 AND id = $2`, [tenantId, id])
...
export async function deletePromptSuggestion(req: Request, res: Response) {
  try {
    const tenantId = await ensureSystemTenantId()
    const id = String(req.params.id || "")
    const r = await query(`DELETE FROM prompt_suggestions WHERE tenant_id = $1 AND id = $2 RETURNING id`, [tenantId, id])
    if (r.rows.length === 0) return res.status(404).json({ message: "Not found" })
