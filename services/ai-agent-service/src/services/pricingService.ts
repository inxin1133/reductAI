 ...
 */
export async function lookupImageTokenPricing(
  providerSlug: string,
  modelKey: string,
  modelId?: string | null,
): Promise<ImageTokenPricing> {
  ...
  async function fetchPricing(tokenCategory: "text" | "image"): Promise<ModelPricing> {
    try {
      type Row = { usage_kind: string; unit_size: number; rate_value: number }
      let r: { rows: Row[] } | null = null
...
): { inputCost: number; cachedInputCost: number; outputCost: number; totalCost: number; currency: string } {
  const unitSize = pricing.text.unitSize || 1_000_000
...
export async function lookupImagePricing(
  ...
  const queryResolution =
    isGoogle && resolution && String(resolution).trim() && String(resolution).toLowerCase() !== "auto"
 ...
 * @returns USD per second. 조회 실패 시 0
 */
export async function lookupMusicPricing(
  providerSlug: string,
  modelKey: string,
  modelId?: string | null,
): Promise<number> {
