...
export async function googleSimulateChat(args: {
  ...
  input: string
  maxTokens: number
  /** text 모델 멀티모달: 첨부 이미지를 vision으로 전달 */
  image_data_urls?: string[]
  ...
  for (const dataUrl of imageUrls) {
    ...
    if (m) imageParts.push({ inlineData: { mimeType: m[1] || "image/png", data: m[2] || "" } })
  }
 ...
 */
export async function googleGenerateImage(args: {
  apiBaseUrl: string
  apiKey: string
  model: string
  prompt: string
  ...
  if (Array.isArray(responseParts)) {
    for (const p of responseParts) {
      ...
      const inline = obj.inlineData && typeof obj.inlineData === "object" ? (obj.inlineData as Record<string, unknown>) : null
  ...
  try {
    if (Array.isArray(rawSafe.candidates)) {
      rawSafe.candidates = (rawSafe.candidates as unknown[]).map((c) => {
        if (!c || typeof c !== "object") return c
        const cc = { ...(c as Record<string, unknown>) }
        const content = cc.content && typeof cc.content === "object" ? (cc.content as Record<string, unknown>) : null
        if (content && Array.isArray(content.parts)) {
          content.parts = (content.parts as unknown[]).map((p) => {
            if (!p || typeof p !== "object") return p
            const pp = { ...(p as Record<string, unknown>) }
            if (pp.inlineData && typeof pp.inlineData === "object") {
              const id = pp.inlineData as Record<string, unknown>
              if (typeof id.data === "string") id.data = `<omitted:${id.data.length}>`
            }
            return pp
          })
        }
...
export async function openaiSimulateChat(args: {
  ...
  const userContent: string | Array<{ type: "text"; text: string } | { type: "image_url"; image_url: { url: string } }> = hasImages
    ? [
  ...
  async function tryResponsesWithNonEmptyText(bodies: Array<unknown>) {
    for (const body of bodies) {
      ...
      if (text) {
        ...
        return { ok: true as const, raw: r.json, output_text: text, truncated: truncated || undefined }
      }
  ...
  if (!skipResponsesForMultimodal && args.outputFormat === "block_json") {
    ...
    if (tried.ok) return { raw: tried.raw, output_text: tried.output_text, truncated: tried.truncated }
...
export async function anthropicSimulateChat(args: {
  ...
  for (const dataUrl of imageUrls) {
    if (!dataUrl || !dataUrl.startsWith("data:image/")) continue
    const m = dataUrl.match(/^data:([^;]+);base64,(.*)$/)
    if (m) imageBlocks.push({ type: "image", source: { type: "base64", media_type: m[1] || "image/png", data: m[2] || "" } })
  }
  const userContent: string | Array<{ type: "text"; text: string } | { type: "image"; source: { type: "base64"; media_type: string; data: string } }> =
    imageBlocks.length > 0 ? [{ type: "text", text: args.input }, ...imageBlocks] : args.input
