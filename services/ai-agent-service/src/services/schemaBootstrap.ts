...
export async function ensureAiAccessSchema() {
  ...
  await query(`
    ...
    END $$;
  `)
  await query(`CREATE INDEX IF NOT EXISTS idx_ai_providers_sort_order ON ai_providers(sort_order);`)
 ...
 */
export async function ensureLlmUsageLogsSchema() {
  await query(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`)
  ...
  await query(`ALTER TABLE llm_usage_logs DROP CONSTRAINT IF EXISTS llm_usage_logs_status_check;`)
  await query(`
    ALTER TABLE llm_usage_logs
  ...
  await query(`DROP INDEX IF EXISTS idx_llm_usage_logs_tenant_request_id;`)
  await query(`CREATE UNIQUE INDEX idx_llm_usage_logs_tenant_request_id ON llm_usage_logs(tenant_id, request_id);`)
  await query(
    `CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_provider_request_id
     ON llm_usage_logs(provider_id, request_id)
     WHERE request_id IS NOT NULL;`
  )
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_tenant_id ON llm_usage_logs(tenant_id);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_user_id ON llm_usage_logs(user_id);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_provider_id ON llm_usage_logs(provider_id);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_model_id ON llm_usage_logs(model_id);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_feature_name ON llm_usage_logs(feature_name);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_modality ON llm_usage_logs(modality);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_resolved_model ON llm_usage_logs(resolved_model);`)
  await query(
    `CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_conversation_id
     ON llm_usage_logs(conversation_id) WHERE conversation_id IS NOT NULL;`
  )
  await query(
    `CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_model_message_id
     ON llm_usage_logs(model_message_id) WHERE model_message_id IS NOT NULL;`
  )
  await query(
    `CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_routing_rule_id
     ON llm_usage_logs(routing_rule_id) WHERE routing_rule_id IS NOT NULL;`
  )
  await query(
    `CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_parent_usage_log_id
     ON llm_usage_logs(parent_usage_log_id) WHERE parent_usage_log_id IS NOT NULL;`
  )
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_status ON llm_usage_logs(status);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_created_at ON llm_usage_logs(created_at);`)
  await query(`CREATE INDEX IF NOT EXISTS idx_llm_usage_logs_tenant_date ON llm_usage_logs(tenant_id, created_at DESC);`)
 ...
 */
export async function ensureWebSearchSettingsSchema() {
  await query(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`)
