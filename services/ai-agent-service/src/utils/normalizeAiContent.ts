...
export function normalizeAiContent(content: unknown): JsonRecord {
  ...
  const responseArr = Array.isArray(content.response) ? (content.response as unknown[]) : null
  if (responseArr && responseArr.length > 0) {
    const responseBlocks = responseArr
      .map((item) => {
        ...
        if (md) return { type: "markdown" as const, markdown: md }
        const code = typeof obj.code === "string" ? obj.code.trim() : typeof obj.content === "string" ? obj.content.trim() : ""
        if (code) return { type: "code" as const, code, language: typeof obj.language === "string" ? obj.language : "plain" }
        const table = coerceTableFromObject(isRecord(obj) ? obj : null)
        if (table) return table
        return null
      })
      .filter(Boolean) as JsonRecord[]
    if (responseBlocks.length > 0) return finalizeNormalizedContent(content, responseBlocks)
  }
