...
exports.getOwnerTenantTier = getOwnerTenantTier;
exports.getCurrentTenant = getCurrentTenant;
exports.listTenantMemberships = listTenantMemberships;
exports.listTenantMembers = listTenantMembers;
exports.updateTenantMember = updateTenantMember;
exports.listTenantInvitations = listTenantInvitations;
exports.createTenantInvitation = createTenantInvitation;
exports.updateTenantInvitation = updateTenantInvitation;
exports.listMyInvitations = listMyInvitations;
exports.acceptMyInvitation = acceptMyInvitation;
exports.rejectMyInvitation = rejectMyInvitation;
exports.listCurrentUserProviders = listCurrentUserProviders;
exports.listCurrentUserSessions = listCurrentUserSessions;
exports.revokeOtherUserSessions = revokeOtherUserSessions;
exports.revokeCurrentUserSession = revokeCurrentUserSession;
exports.getCurrentUser = getCurrentUser;
exports.updateCurrentUser = updateCurrentUser;
exports.updateTenantName = updateTenantName;
...
async function insertAuditLog(args) {
    const { client, tenantId, userId, action, resourceType, resourceId, status, req, requestData, responseData, errorMessage, } = args;
    try {
...
async function getUserProfile(userId) {
    const r = await (0, db_1.query)(`SELECT id, email, full_name FROM users WHERE id = $1 LIMIT 1`, [userId]);
    return r.rows[0];
}
async function resolveTenantManagerRole(userId, tenantId) {
...
async function sendInvitationEmail(args) {
    ...
    const inviteLink = `${APP_BASE_URL}/?invite_email=${encodeURIComponent(inviteeEmail)}&invite_token=${encodeURIComponent(invitationToken)}`;
...
async function refreshTenantMemberCount(client, tenantId) {
...
async function resolvePersonalTenantId(req) {
    const userId = req.userId ? String(req.userId) : "";
    if (!userId)
...
async function resolveCategoryContext(categoryId) {
    const r = await (0, db_1.query)(`
...
async function resolveTenantIdForCategoryType(req, categoryType) {
...
async function createPost(req, res) {
    ...
    try {
        ...
        if (parentId) {
            const p = await client.query(`SELECT p.id, p.category_id, p.tenant_id, p.author_id, t.tenant_type
         FROM posts p
        ...
        if (!tenantId) {
            tenantId = await resolvePersonalTenantId(req);
        }
    ...
    try {
        ...
        if (categoryId) {
            ...
            if (ctx.categoryType === "team_page") {
                ...
                if (!ok)
                    return res.json([]);
                const membershipRes = await (0, db_1.query)(`
          SELECT 1
          FROM user_tenant_roles
          WHERE user_id = $1
            AND tenant_id = $2
...
async function listMyPageCategories(req, res) {
    ...
    try {
        ...
        const tenantId = await resolveTenantIdForCategoryType(req, categoryType);
        ...
        const lockKey = categoryType === "team_page"
...
async function updateCategory(req, res) {
    ...
    try {
        ...
        const cur = await (0, db_1.query)(`SELECT id, tenant_id, category_type, COALESCE(user_id, author_id) AS owner_id
       FROM board_categories
       WHERE id = $1 AND deleted_at IS NULL
       LIMIT 1`, [id]);
...
async function deleteCategory(req, res) {
    ...
    try {
        const userId = req.userId;
        const { id } = req.params;
        const cur = await client.query(`SELECT id, tenant_id, category_type, COALESCE(user_id, author_id) AS owner_id
       FROM board_categories
       WHERE id = $1 AND deleted_at IS NULL
       LIMIT 1`, [id]);
        if (cur.rows.length === 0)
...
async function getOwnerTenantTier(req, res) {
    try {
        const userId = req.userId;
        if (!userId)
...
async function listTenantMemberships(req, res) {
    try {
        const userId = req.userId;
        const r = await (0, db_1.query)(`
...
async function listTenantMembers(req, res) {
    try {
        const authed = req;
        ...
        const params = [tenantId];
        const filters = ["utr.tenant_id = $1", "u.deleted_at IS NULL"];
        if (statusRaw) {
...
async function updateTenantMember(req, res) {
    const client = await db_1.default.connect();
    try {
        await client.query("BEGIN");
        const authed = req;
      ...
      `, [actorId, tenantId]);
        ...
        const canManage = actorRole === "owner" || actorRole === "admin" || actorRole === "tenant_owner" || actorRole === "tenant_admin";
        const isSelfTarget = targetUserId && actorId && targetUserId === actorId;
        if (!canManage && !isSelfTarget)
            return res.status(403).json({ message: "Forbidden" });
        ...
        if (input.role_slug !== undefined) {
            ...
            if (roleIdRes.rows.length === 0)
                return res.status(400).json({ message: "Invalid role_slug" });
        ...
        if (input.membership_status !== undefined) {
            ...
            if (status === "inactive") {
                if (!isSelfTarget) {
                    ...
                    const pwRes = await client.query(`SELECT password_hash FROM users WHERE id = $1 AND deleted_at IS NULL`, [actorId]);
                    const passwordHash = pwRes.rows[0]?.password_hash;
                    if (!passwordHash) {
                        return res.status(400).json({ message: "비밀번호가 설정되어 있지 않습니다." });
                    }
                    const isValid = await bcrypt_1.default.compare(String(password), String(passwordHash));
                    if (!isValid) {
...
async function listTenantInvitations(req, res) {
    try {
        const authed = req;
        ...
        if (!TENANT_MANAGER_ROLES.has(roleSlug))
            return res.status(403).json({ message: "Forbidden" });
        await (0, db_1.query)(`
...
async function createTenantInvitation(req, res) {
    ...
    try {
        ...
        if (!inviteeEmail)
            return res.status(400).json({ message: "invitee_email is required" });
        if (!INVITATION_ROLES.has(membershipRole)) {
            return res.status(400).json({ message: "소유자는 초대할 수 없습니다." });
        }
        const tenantRes = await client.query(`SELECT id, name, member_limit FROM tenants WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [tenantId]);
        if (tenantRes.rows.length === 0)
            return res.status(404).json({ message: "Tenant not found" });
        const tenantName = String(tenantRes.rows[0]?.name || "");
        const inviteeRes = await client.query(`SELECT id, email, full_name FROM users WHERE LOWER(email) = $1 AND deleted_at IS NULL LIMIT 1`, [inviteeEmail]);
...
async function updateTenantInvitation(req, res) {
    const client = await db_1.default.connect();
    try {
        await client.query("BEGIN");
        const authed = req;
        ...
        if (!INVITATION_STATUSES.has(nextStatus))
            return res.status(400).json({ message: "invalid status" });
        if (nextStatus !== "cancelled")
            return res.status(400).json({ message: "only cancel is allowed" });
        const updateRes = await client.query(`
      UPDATE tenant_invitations
...
async function listMyInvitations(req, res) {
    try {
      ...
      `, [userId, userEmail]);
        ...
        const filters = ["(ti.invitee_user_id = $1 OR LOWER(ti.invitee_email) = $2)"];
        if (statusFilter) {
...
async function acceptMyInvitation(req, res) {
    const client = await db_1.default.connect();
    try {
        await client.query("BEGIN");
        const authed = req;
      ...
      `, [invitationId]);
        ...
        if (expiresAt && new Date(expiresAt).getTime() < Date.now()) {
            await client.query(`UPDATE tenant_invitations SET status = 'expired', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [invitationId]);
            await client.query("COMMIT");
      ...
      `, [userId, tenantId]);
        ...
        if (!existing || (existing.membership_status && existing.membership_status !== "active")) {
            const limitRes = await client.query(`SELECT member_limit FROM tenants WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [tenantId]);
...
async function rejectMyInvitation(req, res) {
    const client = await db_1.default.connect();
    try {
        await client.query("BEGIN");
        const authed = req;
...
async function listCurrentUserProviders(req, res) {
    try {
        const userId = req.userId;
        const r = await (0, db_1.query)(`
      SELECT
...
async function listCurrentUserSessions(req, res) {
    try {
        const userId = req.userId;
...
async function revokeOtherUserSessions(req, res) {
    try {
        const userId = req.userId;
        const tokenHash = req.sessionTokenHash || "";
        if (!tokenHash)
            return res.status(400).json({ message: "Current session not found" });
        const result = await (0, db_1.query)(`DELETE FROM user_sessions WHERE user_id = $1 AND token_hash <> $2 RETURNING id`, [userId, tokenHash]);
...
async function revokeCurrentUserSession(req, res) {
    try {
        const userId = req.userId;
        const id = String(req.params.id || "").trim();
        if (!id)
            return res.status(400).json({ message: "Session id is required" });
        const result = await (0, db_1.query)(`DELETE FROM user_sessions WHERE id = $1 AND user_id = $2 RETURNING id`, [id, userId]);
...
async function updateCurrentUser(req, res) {
    try {
        ...
        let profileImageAssetId = undefined;
        ...
        if (profileImageAssetId !== undefined) {
            if (profileImageAssetId === null) {
                ...
                fields.push(`metadata = jsonb_set(COALESCE(metadata, '{}'::jsonb), '{profile_image_asset_id}', to_jsonb($${params.length}::text), true)`);
            }
        }
      ...
      `, params);
        ...
        if (hasMarketingChange) {
            ...
            await (0, db_1.query)(`INSERT INTO user_policy_consents (user_id, policy_type, policy_version, agreed, source, ip_address, user_agent)
         VALUES ($1, 'marketing', '2026-03-02', $2, 'settings', $3, $4)`, [userId, marketingAgreedRaw, ip || null, ua || null]);
        }
...
async function updateTenantName(req, res) {
    try {
        const userId = req.userId;
      ...
      `, [userId, tenantId]);
        ...
        if (!canManage)
            return res.status(403).json({ message: "Forbidden" });
        const r = await (0, db_1.query)(`UPDATE tenants SET name = $1, updated_at = NOW() WHERE id = $2 AND deleted_at IS NULL RETURNING id, name, tenant_type`, [nextName, tenantId]);
        if (r.rows.length === 0)
            return res.status(404).json({ message: "Tenant not found" });
        return res.json(r.rows[0]);
    }
...
async function updatePostCategory(req, res) {
    ...
    try {
        ...
        const body = (req.body || {});
        const categoryId = typeof body.category_id === "string" && body.category_id.trim() ? body.category_id.trim() : null;
        const cur = await client.query(`SELECT id, tenant_id
       FROM posts
...
async function getDeletedPageDetail(req, res) {
    try {
        ...
        const p = await (0, db_1.query)(`SELECT id, tenant_id, parent_id, title, icon, category_id, metadata, status, deleted_at, updated_at
       FROM posts
       WHERE id = $1 AND author_id = $2
       LIMIT 1`, [pageId, userId]);
        if (p.rows.length === 0)
            return res.status(404).json({ message: "Post not found" });
        const row = p.rows[0];
        const tenantId = String(row.tenant_id);
...
async function restoreDeletedPage(req, res) {
    ...
    try {
        ...
        const cur = await client.query(`SELECT id, tenant_id, parent_id, category_id, metadata, status, deleted_at
       FROM posts
       WHERE id = $1 AND author_id = $2
       FOR UPDATE`, [pageId, userId]);
        if (cur.rows.length === 0) {
            await client.query("ROLLBACK");
            return res.status(404).json({ message: "Post not found" });
        }
...
async function purgeDeletedPage(req, res) {
    ...
    try {
        ...
        if (!pageId)
            return res.status(400).json({ message: "id is required" });
        await client.query("BEGIN");
        const cur = await client.query(`SELECT id, tenant_id, status, deleted_at
       FROM posts
       WHERE id = $1 AND author_id = $2
       FOR UPDATE`, [pageId, userId]);
        if (cur.rows.length === 0) {
...
async function movePage(req, res) {
    ...
    try {
        ...
        const pageRes = await client.query(`SELECT id, parent_id, category_id, page_order, tenant_id
       FROM posts
       WHERE id = $1 AND author_id = $2 AND deleted_at IS NULL
       LIMIT 1`, [id, userId]);
