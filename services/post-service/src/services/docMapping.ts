...
function extractTextFromPm(node: any): string {
  ...
  if (node.type === "mention") return node?.attrs?.label ? `@${node.attrs.label}` : "@mention"
  if (node.type === "page_link") return node?.attrs?.title ? String(node.attrs.title) : "page"
  const parts: string[] = []
...
export function blocksToDocJson(blocks: StoredBlock[]): PmJson {
  const content: any[] = []
  for (const b of blocks) {
    const pmNode = (b?.content && (b.content.pm || b.content)) || null
    if (pmNode) content.push(pmNode)
  }
  return { type: "doc", content }
}

export function docJsonToBlocks(args: {
  postId: string
  docJson: PmJson
  pmSchemaVersion?: number
}): Array<{
  parent_block_id: string | null
  block_type: string
  sort_key: number
  ...
  for (const node of children) {
    ...
    let refPostId: string | null = null
    let externalEmbedId: string | null = null

    if (blockType === "page_link") {
      const pid = node?.attrs?.pageId
      if (typeof pid === "string" && pid.length > 0) refPostId = pid
    }
    if (blockType === "external_embed") {
      const eid = node?.attrs?.externalEmbedId
      if (typeof eid === "string" && eid.length > 0) externalEmbedId = eid
    }
