...
exports.cloneRateCard = cloneRateCard;
exports.listRates = listRates;
exports.updateRate = updateRate;
exports.bulkUpdateRates = bulkUpdateRates;
...
exports.generateSkusForModel = generateSkusForModel;
exports.listMarkups = listMarkups;
...
async function listPublicPrices(req, res) {
    try {
        ...
        const countRes = await (0, db_1.query)(`SELECT COUNT(*)::int AS total FROM pricing_model_cost_summaries ${whereSql}`, params);
...
async function listRateCards(req, res) {
    try {
        ...
        const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
        const countRes = await (0, db_1.query)(`SELECT COUNT(*)::int AS total FROM pricing_rate_cards ${whereSql}`, params);
        const listRes = await (0, db_1.query)(`
...
async function createRateCard(req, res) {
    try {
        ...
        if (!Number.isFinite(version) || version <= 0)
            return res.status(400).json({ message: "version must be positive" });
        if (!effectiveAt)
            return res.status(400).json({ message: "effective_at is required" });
        if (!RATE_CARD_STATUSES.has(status))
...
async function cloneRateCard(req, res) {
...
async function bulkUpdateRates(req, res) {
    try {
        const rateCardId = toStr(req.body?.rate_card_id);
        if (!rateCardId)
            return res.status(400).json({ message: "rate_card_id is required" });
...
function buildSkuCode(providerSlug, modelKey, modality, usageKind, tokenCategory, metadata) {
    const base = `${providerSlug}.${modelKey}.${modality}.${usageKind}`;
    if (tokenCategory)
        return `${base}.${tokenCategory}`;
...
async function checkSkuCodeAvailability(req, res) {
    try {
...
async function createSku(req, res) {
    try {
        ...
        if (!providerSlug)
            return res.status(400).json({ message: "provider_slug is required" });
        if (!modelKey)
            return res.status(400).json({ message: "model_key is required" });
        if (!modelName)
        ...
        if (!VALID_USAGE_KINDS.has(usageKind))
            return res.status(400).json({ message: "invalid usage_kind" });
        if (tokenCategory && !VALID_TOKEN_CATEGORIES.has(tokenCategory))
            return res.status(400).json({ message: "invalid token_category" });
        if (!VALID_UNITS.has(unit))
            return res.status(400).json({ message: "invalid unit" });
        if (unitSize <= 0)
            return res.status(400).json({ message: "unit_size must be positive" });
        ...
        if (bodyModelId && typeof bodyModelId === "string" && bodyModelId.trim()) {
            ...
            const modelRes = await (0, db_1.query)(`SELECT id FROM ai_models WHERE model_id = $1 OR name = $1 LIMIT 1`, [modelKey]);
...
async function deactivateSku(req, res) {
    try {
        ...
        if (!id)
            return res.status(400).json({ message: "id is required" });
        const result = await (0, db_1.query)(`UPDATE pricing_skus SET is_active = FALSE, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id, sku_code, is_active`, [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "SKU not found" });
        return res.json({ ok: true, row: result.rows[0] });
    }
...
// ========================================
async function listMissingSkus(req, res) {
    try {
        const rateCardId = String(req.params.id || "");
        if (!rateCardId)
            return res.status(400).json({ message: "rate card id is required" });
        const q = toStr(req.query.q);
        const providerSlug = toStr(req.query.provider_slug);
        const modality = toStr(req.query.modality);
        const where = [
...
async function addRatesToCard(req, res) {
    const client = await db_1.default.connect();
    try {
        const rateCardId = String(req.params.id || "");
        if (!rateCardId)
...
async function checkModelNeedsSkuGeneration(req, res) {
    try {
      ...
      `, [modelId]);
        ...
        const existingCodes = new Set(existRes.rows.map((r) => r.sku_code));
        for (const tpl of templates) {
            const expectedCode = buildSkuCode(providerSlug, modelKey, modality, tpl.usage_kind, tpl.token_category, tpl.metadata);
...
async function generateSkusForModel(req, res) {
    try {
      ...
      `, [modelId]);
        ...
        const skipped = [];
        for (const tpl of templates) {
            const skuCode = buildSkuCode(model.provider_slug || "unknown", model.model_key, modality, tpl.usage_kind, tpl.token_category, tpl.metadata);
            try {
                const result = await (0, db_1.query)(`
          INSERT INTO pricing_skus (
            sku_code, provider_slug, model_id, model_key, model_name,
...
// Markups
// ========================================
async function listMarkups(req, res) {
    try {
        const q = toStr(req.query.q);
        ...
        const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
        const countRes = await (0, db_1.query)(`SELECT COUNT(*)::int AS total FROM pricing_markup_rules ${whereSql}`, params);
        const listRes = await (0, db_1.query)(`
      SELECT
        m.id,
        m.name,
...
async function createMarkup(req, res) {
    try {
        const name = toStr(req.body?.name);
        ...
        if (!MARKUP_SCOPE_TYPES.has(scopeType))
            return res.status(400).json({ message: "invalid scope_type" });
        const result = await (0, db_1.query)(`
...
async function deleteMarkup(req, res) {
    try {
        const id = String(req.params.id || "");
        if (!id)
            return res.status(400).json({ message: "id is required" });
        const result = await (0, db_1.query)(`DELETE FROM pricing_markup_rules WHERE id = $1 RETURNING id`, [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "Markup not found" });
