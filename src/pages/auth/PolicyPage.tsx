...
function Section({ section }: { section: PolicySection }) {
  const Tag = section.level === "h1" ? "h1" : section.level === "h2" ? "h2" : "h3"
  const cls =
    section.level === "h1"
      ? "text-2xl font-bold text-foreground"
      : section.level === "h2"
        ? "text-lg font-semibold text-foreground mt-10 first:mt-0"
        : "text-base font-medium text-foreground mt-7 first:mt-0"
  return (
    <div>
      <Tag className={cls}>{section.title}</Tag>
      {section.paragraphs?.map((p, i) => (
        <p key={i} className="mt-3 text-sm leading-relaxed text-muted-foreground whitespace-pre-wrap">{p}</p>
      ))}
      {section.items && section.items.length > 0 ? (
        <ol className="mt-3 space-y-1.5 text-sm leading-relaxed text-muted-foreground list-decimal list-inside">
...
function BulletList({ items }: { items: string[] }) {
  return (
    <ul className="mt-2 space-y-1 text-sm leading-relaxed text-muted-foreground list-disc list-inside">
      {items.map((item, i) => (
        <li key={i} className="pl-1">{item}</li>
      ))}
    </ul>
...
function TermsContent() {
  const sections: PolicySection[] = [
    ...
    },
    {
      ...
      items: [
        '"서비스"란 회사가 웹/앱 등 온라인 시스템을 통해 제공하는 LLM 기반 생성형 기능(텍스트/코드/이미지/오디오/비디오), 프로젝트/페이지 관리, 공유, 저장공간, 크레딧 기반 과금, 고객지원 등 일체를 말합니다.',
        '"회원"이란 본 약관에 동의하고 회사가 정한 절차에 따라 회원가입을 완료하여 서비스 이용 자격을 부여받은 개인 또는 법인/단체를 말합니다.',
        '"계정"이란 회원 식별 및 서비스 이용을 위해 생성되는 로그인 단위를 말합니다.',
        '"테넌트(Tenant)"란 서비스 내에서 권한/데이터/크레딧/저장공간이 구분되는 작업 공간 단위를 말하며, personal / team / group 유형이 있습니다.',
        '"테넌트 권한"이란 테넌트 내 역할을 의미하며 owner(소유자), admin(관리자), member(멤버), viewer(뷰어)로 구분됩니다.',
        '"서비스 등급(플랜)"이란 free / pro / premium / business / enterprise(향후 제공) 등 회사가 정한 구독 등급을 말합니다.',
        '"크레딧(Credit)"이란 서비스 이용 대가 산정 및 결제를 위해 회사가 부여·판매하는 서비스 내 단위를 말하며, 월 기본 크레딧(Subscription Credits)과 추가 충전 크레딧(Top-up Credits)으로 구분됩니다.',
        '"월 기본 크레딧"이란 구독 등급에 따라 매월 제공되는 크레딧을 말합니다.',
        '"추가 충전 크레딧"이란 회원이 별도로 구매하여 충전하는 크레딧을 말하며, 회사가 정한 사용기한 및 환불 제한이 적용됩니다.',
        '"LLM 사용 로그"란 모델명, 입력/출력/캐시 토큰, 이미지/오디오/비디오/검색 등의 과금 요소, 처리시간 등 비용 산정 및 운영을 위해 기록되는 이용내역을 말합니다.',
        '"운영 환율"이란 결제 시 USD 기준 요금을 각 나라의 환율로 환산하기 위해 회사가 정한 기준 환율을 말합니다.',
        '"콘텐츠"란 회원이 서비스에 업로드/작성/생성한 텍스트, 이미지, 파일, 링크, 메시지, 프로젝트, 페이지, 결과물 및 메타데이터를 말합니다.',
      ],
    },
    {
    ...
    { title: "제 4 조 (약관 외 준칙)", level: "h3", paragraphs: ["본 약관에 명시되지 않은 사항은 관계 법령 및 회사가 정한 운영정책, 공지사항, 서비스별 안내에 따릅니다."] },
    {
      ...
      level: "h3",
      paragraphs: ["① 회사가 제공하는 주요 서비스는 다음과 같습니다."],
      items: [
        "LLM 기반 생성 기능: 텍스트/코드/이미지/오디오/비디오 생성 및 편집 기능",
        "테넌트 및 협업 기능: personal/team/group 테넌트 운영, 초대, 권한 관리, 공유 페이지 제공",
        "크레딧 및 과금 기능: 크레딧 부여/차감, 사용내역 제공, 요금제 관리",
        "저장 및 스토리지 기능: 파일/페이지/자산 저장 및 용량 관리",
        "고객지원: 문의 접수/답변, 공지사항, FAQ 등",
        "기타 회사가 추가로 제공하는 기능",
      ],
    },
    {
      title: "제 6 조 (서비스의 중단)",
      level: "h3",
      paragraphs: ["① 회사는 다음 사유가 있는 경우 서비스 제공을 일시 중단할 수 있습니다."],
      items: [
        "설비 보수/점검/교체/장애",
        "통신사 또는 클라우드 사업자 장애",
        "이용 폭주, 보안 사고 대응",
    ...
    {
      title: "제 9 조 (이용 계약의 지연 및 취소)",
      level: "h3",
      paragraphs: ["① 다음 사유가 있는 경우 가입 승인이 지연될 수 있습니다."],
      items: [
        "기술상 장애",
        "운영 정책상 제한이 필요한 경우",
      ],
    ...
    { title: "제 13 조 (회사의 의무)", level: "h3", items: ["회사는 특별한 사정이 없는 한 안정적 서비스 제공을 위해 노력합니다.", "회사는 회원의 불만/문의에 대해 합리적 절차에 따라 처리하며 필요 시 처리 일정 등을 안내합니다.", "회사는 회원 콘텐츠의 진실성·정확성·완전성을 보증하지 않습니다."] },
    {
      title: "제 14 조 (회원의 의무)",
      level: "h3",
      paragraphs: ["① 회원은 관련 법령, 본 약관, 서비스 내 안내 및 공지 사항을 준수해야 합니다.\n② 회원은 다음 행위를 해서는 안 됩니다."],
      items: [
        "불법/유해 콘텐츠 생성 또는 유포, 타인 권리 침해",
        "해킹, 악성코드 유포, 서비스 부하 유발 등 운영 방해",
        "계정 도용/공유 등 부정 이용",
        "회사의 사전 허락 없는 영리 목적 재판매/재배포(허용된 범위 제외)",
        "타인의 개인정보 수집/저장/유포",
        "기타 공서양속 또는 법령 위반 행위",
      ],
    },
    ...
    },
    {
      ...
      items: [
        ...
        "크레딧 사용 순서는 원칙적으로 회원이 선택한 기본 크레딧 → 잔여 기본 크레딧 → 충전 크레딧 순입니다.",
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="mt-10 border-t border-border/40 pt-6">
        <p className="text-sm text-muted-foreground">본 약관은 2026년 3월 2일부터 시행합니다.</p>
      </div>
    </div>
  )
}
...
function PrivacyContent() {
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-base font-medium text-foreground">1. 수집 항목</h3>
        <div className="mt-3 space-y-2">
          <div className="rounded-lg bg-muted/50 px-4 py-3">
            <span className="text-xs font-semibold text-foreground">필수</span>
            <p className="mt-1 text-sm text-muted-foreground">이메일(아이디), 이름/닉네임(표시명), 비밀번호(암호화), 결제 처리 관련 식별 정보(결제 토큰 등), 고객지원 문의 내용</p>
          </div>
          <div className="rounded-lg bg-muted/50 px-4 py-3">
            <span className="text-xs font-semibold text-foreground">자동 수집</span>
            <p className="mt-1 text-sm text-muted-foreground">접속 IP, 기기/브라우저 정보, 쿠키/로그, 서비스 이용 기록(테넌트/권한/설정), LLM 사용 로그(모델명, 토큰/모달리티 사용량, 과금 요소, 처리 시간 등)</p>
          </div>
        </div>
      </div>

      <div>
        <h3 className="text-base font-medium text-foreground">2. 이용 목적</h3>
        <BulletList items={[
          "회원 가입 및 본인 식별, 계정 관리",
      ...
      <div>
        <h3 className="text-base font-medium text-foreground">3. 보관 기간</h3>
        <BulletList items={[
      ...
      <div>
        <h3 className="text-base font-medium text-foreground">4. 제3자 제공/처리위탁</h3>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          서비스 제공을 위해 결제 처리사(예: 토스, Stripe) 및 AI 모델/API 제공사에 일부 정보가 제공되거나 처리될 수 있습니다. 제공/위탁 범위 및 항목은 개인정보처리방침에 따릅니다.
        </p>
      </div>

      <div className="rounded-lg border border-amber-200 bg-amber-50/50 px-4 py-3 dark:border-amber-800 dark:bg-amber-950/30">
      ...
      <div className="border-t border-border/40 pt-6">
        <p className="text-sm text-muted-foreground">시행일: 2026년 3월 2일</p>
      </div>
    </div>
  )
}
...
function CookiesContent() {
  const sections: PolicySection[] = [
    { title: "제1조 (쿠키의 정의)", level: "h2", paragraphs: ["쿠키(Cookie)란 웹사이트가 이용자의 브라우저에 저장하는 작은 텍스트 파일로, 서비스 이용 시 이용자의 기기 내에 저장되며 서비스 운영에 필요한 정보를 포함합니다."] },
    { title: "제2조 (쿠키의 사용 목적)", level: "h2", paragraphs: ["회사는 다음 목적을 위해 쿠키를 사용합니다."] },
  ]
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {[
          { title: "1. 필수 쿠키", desc: "Essential Cookies", items: ["로그인 세션 유지", "계정 인증 상태 유지", "보안 및 비인가 사용 방지", "테넌트(personal / team / group) 식별"], note: "필수 쿠키는 서비스 제공에 반드시 필요합니다.", accent: "border-blue-200 dark:border-blue-800" },
          { title: "2. 기능 쿠키", desc: "Functional Cookies", items: ["사용자 환경설정 저장", "언어 설정", "테마(다크모드 등) 저장"], accent: "border-teal-200 dark:border-teal-800" },
          { title: "3. 분석 쿠키", desc: "Analytics Cookies", items: ["방문 통계 분석", "이용 패턴 분석", "서비스 개선 목적"], accent: "border-violet-200 dark:border-violet-800" },
          { title: "4. 마케팅 쿠키", desc: "Marketing Cookies (선택 동의)", items: ["신규 서비스 안내", "이벤트/프로모션 안내", "광고 효과 측정"], accent: "border-amber-200 dark:border-amber-800" },
        ].map((cat) => (
          <div key={cat.title} className={cn("rounded-xl border bg-card px-4 py-4", cat.accent)}>
            ...
            {cat.note ? <p className="mt-2 text-[11px] text-muted-foreground">{cat.note}</p> : null}
          </div>
        ))}
      </div>

      <Section section={{ title: "제3조 (쿠키의 보유 기간)", level: "h2" }} />
      <BulletList items={["세션 쿠키: 브라우저 종료 시 삭제", "영구 쿠키: 최대 2년 또는 설정 목적 달성 시까지"]} />

      <Section section={{ title: "제4조 (쿠키 설정 거부 방법)", level: "h2", paragraphs: ["이용자는 브라우저 설정을 통해 쿠키 저장을 거부할 수 있습니다."] }} />
      <div className="mt-2 grid gap-2 sm:grid-cols-3">
        {[
          { browser: "Chrome", path: "설정 → 개인정보 및 보안 → 쿠키 및 기타 사이트 데이터" },
          { browser: "Safari", path: "설정 → 개인정보 보호 → 쿠키 차단" },
          { browser: "Edge", path: "설정 → 쿠키 및 사이트 권한" },
        ].map((b) => (
          <div key={b.browser} className="rounded-lg bg-muted/50 px-3 py-2.5">
            <div className="text-xs font-semibold text-foreground">{b.browser}</div>
            <div className="mt-0.5 text-[11px] text-muted-foreground">{b.path}</div>
          </div>
        ))}
      </div>
      <p className="mt-2 text-sm text-muted-foreground">단, 필수 쿠키를 차단할 경우 로그인 및 서비스 이용이 제한될 수 있습니다.</p>

      <Section section={{ title: "제5조 (제3자 쿠키)", level: "h2", paragraphs: ["회사는 서비스 분석 및 운영을 위해 제3자 도구를 사용할 수 있으며, 해당 업체가 쿠키를 설정할 수 있습니다. 이 경우 해당 업체의 개인정보 처리방침이 적용됩니다."] }} />
      <Section section={{ title: "제6조 (쿠키 정책 변경)", level: "h2", paragraphs: ["본 정책은 관련 법령 또는 서비스 변경에 따라 수정될 수 있으며, 변경 시 서비스 내 공지합니다."] }} />
    </div>
  )
}
...
function RefundContent() {
  ...
  return (
    <div className="space-y-2">
      ...
      <h2 className="text-lg font-semibold text-foreground mt-10">제4조 (구독의 해지)</h2>

      <h3 className="text-base font-medium text-foreground mt-7">① 월간 구독 해지</h3>
      <ol className="mt-3 space-y-1.5 text-sm leading-relaxed text-muted-foreground list-decimal list-inside">
      ...
      <h3 className="text-base font-medium text-foreground mt-7">② 연간 구독 해지 및 환불</h3>
      <ol className="mt-3 space-y-1.5 text-sm leading-relaxed text-muted-foreground list-decimal list-inside">
      ...
      {[
        ...
        <div key={group.sub}>
          <h3 className="text-base font-medium text-foreground mt-7">{group.sub}</h3>
          <ol className="mt-3 space-y-1.5 text-sm leading-relaxed text-muted-foreground list-decimal list-inside">
            {group.items.map((item, i) => <li key={i} className="pl-1">{item}</li>)}
          </ol>
        </div>
      ))}
      ...
      <h2 className="text-lg font-semibold text-foreground mt-10">제8조 (환불의 제한)</h2>
      <p className="mt-3 text-sm leading-relaxed text-muted-foreground">다음 각 호의 경우 환불이 제한될 수 있다.</p>
      <ol className="mt-3 space-y-1.5 text-sm leading-relaxed text-muted-foreground list-decimal list-inside">
...
export default function PolicyPage() {
  ...
  const current = POLICY_NAV.find((n) => n.to === location.pathname)
  const title = current?.label ?? "정책"
  const ContentComponent = CONTENT_MAP[location.pathname]
          ...
          <main className="min-w-0 flex-1">
            ...
            <div className="mt-8 rounded-xl border border-border/60 bg-card p-6 sm:p-8">
              {ContentComponent ? <ContentComponent /> : (
                <p className="text-muted-foreground">{title} 내용이 준비 중입니다.</p>
              )}
            </div>
          </main>
        </div>
